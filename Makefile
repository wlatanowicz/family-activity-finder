...
test-be: ## Run backend tests with ephemeral Postgres (Docker; migrations run inside pytest)
	set -euo pipefail; \
	docker rm -f family-activity-finder-test-db 2>/dev/null || true; \
	docker run -d --rm --name family-activity-finder-test-db -p $(TEST_DB_PORT):5432 \
	  -e POSTGRES_PASSWORD=$(TEST_DB_PASSWORD) postgres:18; \
	trap 'docker kill family-activity-finder-test-db 2>/dev/null || true' EXIT; \
	for _ in $$(seq 1 60); do \
	  docker exec family-activity-finder-test-db pg_isready -U postgres >/dev/null 2>&1 && break; \
	  sleep 0.5; \
	done; \
	docker exec family-activity-finder-test-db pg_isready -U postgres >/dev/null; \
